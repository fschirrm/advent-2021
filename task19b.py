...
max_dist = 0; s1 = -1; s2 = -1
for i in range(len(manh_list)):
    for j in range (len(manh_list)):
        if i != j:
            man_dist = abs(manh_list[i][1][0] - manh_list[j][1][0]) + \
            abs(manh_list[i][1][1] - manh_list[j][1][1]) + abs(manh_list[i][1][2] - manh_list[j][1][2])
            if man_dist > max_dist:
                max_dist = man_dist
                s1 = manh_list[i][0]
                s2 = manh_list[j][0]
print(f"Max. Dist between Scanner {s1} and {s2} is {max_dist}")